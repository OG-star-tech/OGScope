# OGScope documentation index

English | [中文](README.md)

## Start here

- **New to the project / first run**: **A Getting started**.
- **Board networking, hotspot, plate DB, stability, BOM**: **B Board ops**.
- **API, contracts, architecture, tests, pre-commit checklist**: **C API and quality**.
- **External integration (subordinate mode, delegated sensors)**: **D External integration**.
- **Debug console and contributing**: **E Tools and contributing**.

### A Getting started

- [Quick Start Guide](QUICK_START_EN.md) | [中文](QUICK_START.md)
- [Development Guide (board deployment)](development/README_EN.md) | [中文](development/README.md)

### B Board ops

- [WiFi / NetworkManager](development/wifi-nm_EN.md) | [中文](development/wifi-nm.md)
- [Plate-solve data](development/plate-solve-data.md) (inline bilingual sections)
- [Service stability / memory](development/ogscope-service-hardening.md) (inline bilingual sections)
- [Hardware BOM](hardware/bom_EN.md) | [中文](hardware/bom.md)

### C API and quality

- [API layering, FastAPI entry, directory layout](API_ARCHITECTURE_EN.md) | [中文](API_ARCHITECTURE.md)
- [System architecture (bilingual single doc)](architecture/OGSCOPE_SYSTEM_ARCHITECTURE_BILINGUAL.md)
- [Core REST v1](contracts/core-rest-v1_EN.md) | [中文](contracts/core-rest-v1.md)
- [Dev REST v1](contracts/dev-rest-v1_EN.md) | [中文](contracts/dev-rest-v1.md)
- [Core compatibility matrix](contracts/core-compatibility-matrix.md) (inline bilingual sections)
- [Architecture Quick Checklist](development/ARCHITECTURE_QUICK_CHECKLIST_EN.md) | [中文](development/ARCHITECTURE_QUICK_CHECKLIST.md)
- [Testing guide](development/testing-guide_EN.md) | [中文](development/testing-guide.md)

### D External integration

- [Subordinate runtime mode](contracts/subordinate-mode_EN.md) | [中文](contracts/subordinate-mode.md)
- [Hardware Plane UDS v1 (delegated sensors)](contracts/hardware-plane-uds-v1_EN.md) | [中文](contracts/hardware-plane-uds-v1.md)
- [Core REST v1](contracts/core-rest-v1_EN.md) | [中文](contracts/core-rest-v1.md) (primary integrator entry)

### E Tools and contributing

- [Script authoring and invocation standard](development/SCRIPT_STANDARDS_EN.md) | [中文](development/SCRIPT_STANDARDS.md)
- [Debug console](DEBUG_CONSOLE_EN.md) | [中文](DEBUG_CONSOLE.md)
- [Contributing](../CONTRIBUTING_EN.md) | [中文](../CONTRIBUTING.md)
