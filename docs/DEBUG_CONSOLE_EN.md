...
| `camera_v4l2_black_level` / `camera_v4l2_white_level` | `-1` / `0` | RAW levels; negative/zero derives from controls or bit depth |
...
| `camera_v4l2_ae_trace_enabled` | `false` | Explicitly enable bounded AE traces and sampled RAW diagnostics |
| `camera_v4l2_ae_trace_max_events` / `camera_v4l2_ae_trace_max_raw_samples` | `2000` / `100` | Per-session diagnostic persistence limits |
