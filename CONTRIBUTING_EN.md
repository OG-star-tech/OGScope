...
| Branch | Purpose |
|--------|---------|
| `main` | Stable release; promote from `develop` via PR only |
| `develop` | **Long-lived test integration branch**; default for daily work, board sync, and community validation |
| `feature/*` | Features/refactors; branch from `develop`, delete after merge |
| `fix/*` | Small fixes; validate through `develop`; urgent fixes must still merge back through `develop` |

Flow: `feature/*` / `fix/*` → PR → `develop` → validated PR → `main`.

**Do not commit or push directly to `main` or `develop`.** Local pre-commit blocks this; GitHub Rulesets enforce the remote policy. See [.github/scripts/apply-branch-protection.sh](.github/scripts/apply-branch-protection.sh).

**Deprecated**: `staging`, `dev`, and `dev-latest` integration branches.
...
3. Open a PR  
4. Review  
5. Merge into `develop`, then promote to `main` by PR after validation
